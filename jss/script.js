...
document.getElementById('registerBtn').onclick = () => {
    ...
    if (!newUser || !newPass) {
        document.getElementById('loginMsg').innerText = 'Isi semua field!';
        return;
    }
    if (newPass !== confirm) {
        document.getElementById('loginMsg').innerText = 'Password tidak cocok!';
        return;
    }
    const users = JSON.parse(localStorage.getItem('users'));
    if (users.find(u => u.username === newUser)) {
        document.getElementById('loginMsg').innerText = 'Username sudah ada!';
        return;
    }
